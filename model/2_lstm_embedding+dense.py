...
class siameseLSTM(object):
    ...
    def __init__(self, rnn_size, layer_size, sequence_len, grad_clip, learning_rate, decay_rate, decay_step, embedding):
        ...
        with tf.name_scope('embedding'):
            ...
            inputs_x1 = tf.reshape(inputs_x1, [-1, 300])
            inputs_x2 = tf.reshape(inputs_x2, [-1, 300])

        with tf.name_scope('dense_layer1'):
            inputs_x1 = tf.layers.dense(inputs_x1, 150, name='dense1')
            inputs_x2 = tf.layers.dense(inputs_x2, 150, name='dense1', reuse=True)

            inputs_x1 = tf.reshape(inputs_x1, [-1, sequence_len, 150])
            inputs_x2 = tf.reshape(inputs_x2, [-1, sequence_len, 150])

            inputs_x1 = self.transpose_inputs(inputs_x1, 150, sequence_len)
            inputs_x2 = self.transpose_inputs(inputs_x2, 150, sequence_len)
            print('inputs_x1', inputs_x1)           # split:0 ~ split:49   shape=(?, 150)
        ...
        with tf.name_scope('output'):
            ...
            print('outputs_x1：', outputs_x1)        # concat:0 ~ concat_49:0    shape=(?, 300)

            output_x1 = tf.reduce_max(outputs_x1, 0)        # Max:0     shape=(?, 300)
...
with tf.Session() as sess:
    ...
    # saver = tf.train.import_meta_graph('../input/lstm3-v49-epoch2/model.ckpt.meta')
    # ckpt = tf.train.latest_checkpoint('../input/lstm3-v49-epoch2/')
    # if ckpt:
    #     print('加载模型...')
