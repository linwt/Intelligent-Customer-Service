...
with tf.Session() as sess:
    ...
    for rand, stand in zip(test_rand, test_stand):
        ...
        for model in models:
            ...
            indexs.append(label_index[0][0])
        res = Counter(indexs).most_common(1)
        res_index = res[0][0]

        max_sim = 0
        for i, v in enumerate(indexs):
            if v == res_index and sims[i] > max_sim:
                max_sim = sims[i]
        ...
        print('最大相似度：', float('%.4f' %max_sim), '     随机：', test_data[count][0],
              '     标准：', test_data[count][1], '    预测：', standard_data[res_index])
    ...
    print('============= 相似度大于0.8的条件下，', count, '条数据测试结果 =============')
