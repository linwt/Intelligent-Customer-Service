...
class siameseLSTM(object):
    ...
    def bi_lstm(self, rnn_size, layer_size, keep_prob):
        ...
        with tf.name_scope('lstm_fw_rnn'), tf.variable_scope('lstm_fw_rnn'):
        ...
        with tf.name_scope('lstm_bw_rnn'), tf.variable_scope('lstm_bw_rnn'):
    ...
    def bi_gru(self, rnn_size, layer_size, keep_prob):
        ...
        with tf.name_scope('gru_fw_rnn'), tf.variable_scope('gru_fw_rnn'):
            gru_fw_cell_list = [tf.contrib.rnn.GRUCell(rnn_size) for _ in range(layer_size)]
            gru_fw_cell_mul = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.MultiRNNCell(gru_fw_cell_list), output_keep_prob=keep_prob)

        with tf.name_scope('gru_bw_rnn'), tf.variable_scope('gru_bw_rnn'):
            gru_bw_cell_list = [tf.contrib.rnn.GRUCell(rnn_size) for _ in range(layer_size)]
            gru_bw_cell_mul = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.MultiRNNCell(gru_bw_cell_list), output_keep_prob=keep_prob)

        return gru_fw_cell_mul, gru_bw_cell_mul
    ...
    def __init__(self, rnn_size, layer_size, sequence_len, grad_clip, learning_rate, decay_rate, decay_step, embedding):
        ...
        with tf.name_scope('output'):
            ...
            bigru_fw, bigru_bw = self.bi_gru(rnn_size, layer_size, self.dropout_keep_prob)

            # 返回值是一个tuple(outputs, outputs_state_fw, output_state_bw )
            # outputs为一个长度为600的list，每一个元素都包含正向和反向的输出(output_fw, output_bw)
            lstm_outputs_x1, _, _ = tf.contrib.rnn.static_bidirectional_rnn(bilstm_fw, bilstm_bw, inputs_x1, dtype=tf.float32)
            print('outputs_x1：', lstm_outputs_x1)        # concat:0 ~ concat_49:0    shape=(?, 600)

            lstm_output_x1 = tf.reduce_max(lstm_outputs_x1, 0)        # Max:0     shape=(?, 600)
            print('output_x1：', lstm_output_x1)

            gru_outputs_x1, _, _ = tf.contrib.rnn.static_bidirectional_rnn(bigru_fw, bigru_bw, inputs_x1, dtype=tf.float32)
            gru_output_x1 = tf.reduce_mean(gru_outputs_x1, 0)

            # 开启变量重用的开关，共享参数
            tf.get_variable_scope().reuse_variables()

            lstm_outputs_x2, _, _ = tf.contrib.rnn.static_bidirectional_rnn(bilstm_fw, bilstm_bw, inputs_x2, dtype=tf.float32)
            lstm_output_x2 = tf.reduce_max(lstm_outputs_x2, 0)

            gru_outputs_x2, _, _ = tf.contrib.rnn.static_bidirectional_rnn(bigru_fw, bigru_bw, inputs_x2, dtype=tf.float32)
            gru_output_x2 = tf.reduce_mean(gru_outputs_x2, 0)

            # output_x1 = tf.concat([lstm_output_x1, gru_output_x1], 1)
            # output_x2 = tf.concat([lstm_output_x2, gru_output_x2], 1)

            output_x1 = tf.reduce_max([lstm_output_x1, gru_output_x1], 0)
            output_x2 = tf.reduce_max([lstm_output_x2, gru_output_x2], 0)
