...
class siameseLSTM(object):
    ...
    def __init__(self, rnn_size, layer_size, sequence_len, grad_clip, learning_rate, decay_rate, decay_step, embedding):
        ...
        with tf.name_scope('embedding'):
            ...
            inputs_x11 = self.transpose_inputs(inputs_x1, rnn_size, sequence_len)
            inputs_x22 = self.transpose_inputs(inputs_x2, rnn_size, sequence_len)
        ...
        with tf.name_scope('output'):
            ...
            outputs_x1, _, _ = tf.contrib.rnn.static_bidirectional_rnn(bilstm_fw, bilstm_bw, inputs_x11, dtype=tf.float32)
            ...
            outputs_x2, _, _ = tf.contrib.rnn.static_bidirectional_rnn(bilstm_fw, bilstm_bw, inputs_x22, dtype=tf.float32)
            output_x2 = tf.reduce_max(outputs_x2, 0)

        with tf.name_scope('cnn1'):
            cnn_x1_v1 = tf.layers.conv1d(inputs_x1, 100, 1, padding='same', activation=tf.nn.relu, name='c1d_v1')
            cnn_x2_v1 = tf.layers.conv1d(inputs_x2, 100, 1, padding='same', activation=tf.nn.relu, name='c1d_v1', reuse=True)

            max_cnn_x1_v1 = tf.reduce_max(cnn_x1_v1, 1)
            max_cnn_x2_v1 = tf.reduce_max(cnn_x2_v1, 1)

        with tf.name_scope('cnn2'):
            cnn_x1_v2 = tf.layers.conv1d(inputs_x1, 100, 2, padding='same', activation=tf.nn.relu, name='c1d_v2')
            cnn_x2_v2 = tf.layers.conv1d(inputs_x2, 100, 2, padding='same', activation=tf.nn.relu, name='c1d_v2', reuse=True)

            max_cnn_x1_v2 = tf.reduce_max(cnn_x1_v2, 1)
            max_cnn_x2_v2 = tf.reduce_max(cnn_x2_v2, 1)

        with tf.name_scope('cnn3'):
            cnn_x1_v3 = tf.layers.conv1d(inputs_x1, 100, 3, padding='same', activation=tf.nn.relu, name='c1d_v3')
            cnn_x2_v3 = tf.layers.conv1d(inputs_x2, 100, 3, padding='same', activation=tf.nn.relu, name='c1d_v3', reuse=True)

            max_cnn_x1_v3 = tf.reduce_max(cnn_x1_v3, 1)
            max_cnn_x2_v3 = tf.reduce_max(cnn_x2_v3, 1)

        with tf.name_scope('cnn4'):
            cnn_x1_v4 = tf.layers.conv1d(inputs_x1, 100, 4, padding='same', activation=tf.nn.relu, name='c1d_v4')
            cnn_x2_v4 = tf.layers.conv1d(inputs_x2, 100, 4, padding='same', activation=tf.nn.relu, name='c1d_v4', reuse=True)

            max_cnn_x1_v4 = tf.reduce_max(cnn_x1_v4, 1)
            max_cnn_x2_v4 = tf.reduce_max(cnn_x2_v4, 1)

        with tf.name_scope('cnn5'):
            cnn_x1_v5 = tf.layers.conv1d(inputs_x1, 100, 5, padding='same', activation=tf.nn.relu, name='c1d_v5')
            cnn_x2_v5 = tf.layers.conv1d(inputs_x2, 100, 5, padding='same', activation=tf.nn.relu, name='c1d_v5', reuse=True)

            max_cnn_x1_v5 = tf.reduce_max(cnn_x1_v5, 1)
            max_cnn_x2_v5 = tf.reduce_max(cnn_x2_v5, 1)

        with tf.name_scope('cnn6'):
            cnn_x1_v6 = tf.layers.conv1d(inputs_x1, 100, 6, padding='same', activation=tf.nn.relu, name='c1d_v6')
            cnn_x2_v6 = tf.layers.conv1d(inputs_x2, 100, 6, padding='same', activation=tf.nn.relu, name='c1d_v6', reuse=True)

            max_cnn_x1_v6 = tf.reduce_max(cnn_x1_v6, 1)
            max_cnn_x2_v6 = tf.reduce_max(cnn_x2_v6, 1)

        with tf.name_scope('concat'):
            output_x1 = tf.concat([output_x1, max_cnn_x1_v1, max_cnn_x1_v2, max_cnn_x1_v3, max_cnn_x1_v4, max_cnn_x1_v5, max_cnn_x1_v6], 1)
            output_x2 = tf.concat([output_x2, max_cnn_x2_v1, max_cnn_x2_v2, max_cnn_x2_v3, max_cnn_x2_v4, max_cnn_x2_v5, max_cnn_x2_v6], 1)

        with tf.name_scope('dense_layer'):
            fc_w = self.get_weight([4*rnn_size, 128], 'fc_w')
...
tf.flags.DEFINE_integer('layer_size', 4, 'number of layers of RNN')
...
train_x1 = train_x1[:20000]
train_x2 = train_x2[:20000]
train_y = train_y[:20000]

test_rand = test_rand[:200]
test_stand = test_stand[:200]
...
with tf.Session() as sess:
    ...
    # saver = tf.train.import_meta_graph('../input/siamese-atec/model.meta')
    # ckpt = tf.train.latest_checkpoint('../input/siamese-atec/')
    # if ckpt:
    #     print('加载模型...')
